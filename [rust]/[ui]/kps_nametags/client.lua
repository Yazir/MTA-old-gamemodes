	...
	for k,v in ipairs(proximalPlayers) do
		if isElement(v) and getElementData( v, "login" ) then
			...
			if x and dist < 100 and isLineOfSightClear( cx,cy,cz,px,py,pz,true,false,false,true,false,false,false) then 
				...
				if getElementData( v, "isInSafezone" ) then hpR, hpG, hpB = 128,255,0 end
				...
				if armorPerc*100 > 5 then
					...
					dxDrawLine( x, y-(TAG_HEIGHT*3.5)*size, x+(TAG_WIDTH*armorPerc)*size, y-(TAG_HEIGHT*3.5)*size, tocolor( 225, 225, 225, 200*size*aM ), (TAG_HEIGHT)*size )
				end
				...
				dxDrawLine( x, y-(TAG_HEIGHT*2)*size, x+(TAG_WIDTH*hpPerc)*size, y-(TAG_HEIGHT*2)*size, tocolor( hpR, hpG, hpB, 200*size*aM ), (TAG_HEIGHT)*size )
