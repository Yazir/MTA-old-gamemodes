...
class "CGas"
{
	__init__ = function(self, x, y, z, startRadius, endRadius, time)
		...
		for i = 0, 12 do
			...
			local materialSize = 16 * math.max(5, self.radius/20)
			...
			radarEnd = createRadarArea(x+startX, y+startY, radarAreaSize, radarAreaSize, 255, 0, 0, 180)
			...
			local radar = createRadarArea(x+startX, y+startY, radarAreaSize, radarAreaSize, 0, 200, 0, 200)
	...
	resizeToRadius = function(self, radius)
		...
		Async:foreach(self.data, function(v)
			k = k+1
			local startX = math.cos( math.rad( k*30 ) ) * ( self.radius ) 
			local startY = math.sin( math.rad( k*30 ) ) * ( self.radius )
	 
			local materialSize = 50 * math.max(5, self.radius/20)
			...
			if now >= self.radarAreaRefresh then 
				...
				local radarAreaSize =  8 * math.max(3, self.radius/200), 8 * math.max(3, self.radius/200)
				local radar = createRadarArea(self.x+startX, self.y+startY, radarAreaSize, radarAreaSize, 0, 200, 0, 100)
				setElementDimension(radar, getElementDimension(localPlayer))
			
				self.data[k][2] = radar

				if k == 12 then self.radarAreaRefresh = getTickCount()+1000  end 
			end 
		end) 
	...
	onRender = function(self)				
		...
		if dist > self.radius then 
			...
			if now >= self.hpTick then 
				setElementHealth(localPlayer, getElementHealth(localPlayer)-0.1)
				self.hpTick = now+50
			end 
		end 
...
--[[
addCommandHandler( "gas",
function (  )
	local x,y,z = getElementPosition( localPlayer )
	onClientCreateGas( x,y,z, 1000, 30, 0.2 )
end )]]